...
//------------------------------------------------------------------
//The fetch, conversion, and parse function.
function debugAjax() {
    //the fetch function retrieves the data specified at the provided location.  It returns a promise
    fetch("data/MegaCities.geojson")
    //the response object is that returned promise, and it's .then passed to an anonymous function
        .then(function(response) {
            //which converts the response to JSON and returns that result.
            return response.json();
        })
        //data is the return on the first .then method which is passed to the debugCallback function.
        .then(function(data) {
            debugCallback(data);
        })
};

/*This is the AJAX callback function. The purpose of the frunction is to handle the 
response from an AJAX call.*/
function debugCallback(data) {
    //selects the HTML element with ID 'mydiv' for where this will display
    document.querySelector("#mydiv")
        //inserts a line break, the text, and a JSON string version of the data
        .insertAdjacentHTML('beforeend', '<br>GeoJSON Data:<br>' + JSON.stringify(data));
};

//------------------------------------------------------------------
//CREATING THE CITY AND POPULATION TABLE
//------------------------------------------------------------------
